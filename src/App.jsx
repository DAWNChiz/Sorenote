...
// ─── SUPABASE CONFIG ──────────────────────────────────────────────────────────
const SUPABASE_URL = "https://weoslxgtbygtnfqmoatc.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Indlb3NseGd0YnlndG5mcW1vYXRjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU0OTg0ODIsImV4cCI6MjA5MTA3NDQ4Mn0.F108GwTW7P7IA9FFmg1yN6gW-GcZryqruG_Fq4yqhWU";

async function fetchProgress() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/progress?select=topic_id,completed`, {
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
  });
  if (!res.ok) return {};
  const data = await res.json();
  const map = {};
  data.forEach(row => { if (row.completed) map[row.topic_id] = true; });
  return map;
}
...
async function markComplete(topicId) {
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/progress`, {
    ...
    body: JSON.stringify({ topic_id: topicId, completed: true, updated_at: new Date().toISOString() }),
  });
}

async function unmarkComplete(topicId) {
  await fetch(`${SUPABASE_URL}/rest/v1/progress?topic_id=eq.${topicId}`, {
    method: "PATCH",
    headers: {
      apikey: SUPABASE_KEY,
      Authorization: `Bearer ${SUPABASE_KEY}`,
    ...
    body: JSON.stringify({ completed: false, updated_at: new Date().toISOString() }),
  });
}
...
The Convention on the Rights of the Child (CRC) is an international human rights treaty adopted by the United Nations General Assembly on November 20, 1989, and entered into force on September 2, 1990. It is the most widely ratified human rights treaty in history, with 196 state parties.
...
• It also includes members who are retired judges of the Supreme Court and retired Chief Justices of High Courts.
...
India has one of the most comprehensive constitutional and legal frameworks for human rights in the world. Yet a significant gap remains between rights on paper and rights in practice.
...
• Practices like child marriage, dowry, honour killings, and female foeticide violate women's human rights.
• Gender-based violence remains alarmingly high despite strong laws.

**5. Lack of Inclusive Development**
• Economic growth in India has been uneven — large sections including tribal communities, Dalits, women, and the disabled remain excluded.
• Development projects like dams, mines, and industrial plants have led to forced displacement of tribal and rural communities.
...
• In India, laws like UAPA (Unlawful Activities Prevention Act) have been criticized for enabling arbitrary arrest and prolonged detention.
...
• Challenges to the right to health in India include inadequate public health infrastructure, shortage of doctors in rural areas, high cost of private healthcare, and malnutrition.
...
• In India, the Supreme Court has used Article 21 and Article 48A to develop environmental human rights jurisprudence through MC Mehta v. Union of India and other landmark cases.
...
Among the most urgent human rights concerns in India is the systematic discrimination and violence experienced by the most vulnerable sections of society.
...
• Forms of violence include domestic violence, rape and sexual assault, dowry-related violence, acid attacks, honour killings, trafficking, and workplace harassment.
...
• Forms of discrimination include social exclusion, denial of access to public spaces, manual scavenging, violence (atrocities), and economic exploitation.
...
• The National Commission for Minorities (NCM) works to address grievances and safeguard minority rights.
...
**Bonded Labour**
• Bonded labour (debt bondage) is a form of modern slavery in which a person is forced to work to repay a debt under exploitative conditions.
...
// ─── STUDY PLAN ──────────────────────────────────────────────────────────────
const DAILY_PLAN = [
  { day: 1, label: "Day 1", theme: "Unit II — Starting Strong 🌟", topics: [
  ...
  { day: 2, label: "Day 2", theme: "Unit II — International Covenants 📜", topics: [
  ...
  { day: 3, label: "Day 3", theme: "Unit II — Protecting the Vulnerable 💜", topics: [
  ...
  { day: 4, label: "Day 4", theme: "Unit III — Rights at Home 🇮🇳", topics: [
  ...
  { day: 5, label: "Day 5", theme: "Unit III — Justice & The Courts ⚖️", topics: [
    { id: "judiciary", unit: "Unit III", title: "Role of Judiciary in Protection of Human Rights", subtitle: "Courts as guardians of rights" },
  ]},
  { day: 6, label: "Day 6", theme: "Unit IV — Institutions & Civil Society 🏛️", topics: [
  ...
  { day: 7, label: "Day 7", theme: "Unit V — Issues & Concerns 🔥", topics: [
...
function renderNotes(text) {
  return text.split("\n").map((line, i) => {
    if (line.startsWith("## ")) return <h2 key={i} style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.25rem", color:"#d4aaff", borderBottom:"1px solid rgba(180,120,255,0.25)", paddingBottom:10, marginBottom:14 }}>{line.replace("## ","")}</h2>;
    ...
    if (line.startsWith("• ")) return <p key={i} style={{ fontSize:"0.87rem", lineHeight:1.75, color:"rgba(240,234,255,0.88)", paddingLeft:12, marginBottom:8 }}>{line}</p>;
    if (line.trim()==="") return <br key={i}/>;
    return <p key={i} style={{ fontSize:"0.87rem", lineHeight:1.75, color:"rgba(240,234,255,0.75)", marginBottom:10 }}>{line}</p>;
  });
}
...
export default function StudyFlow() {
  ...
  async function handleMarkComplete(topicId) {
    ...
    setCompletedTopics(p => ({ ...p, [topicId]: true }));
    setSavingId(null);
    setShowCelebration(true);
    setTimeout(() => setShowCelebration(false), 2800);
  }

  async function handleUnmark(topicId) {
    setSavingId(topicId);
    await unmarkComplete(topicId);
    setCompletedTopics(p => { const n = { ...p }; delete n[topicId]; return n; });
    setSavingId(null);
  }

  if (loadingProgress) {
    return (
      <div style={{ minHeight:"100vh", background:"#0f0a1e", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16 }}>
        <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;1,400&family=DM+Sans:wght@400;600&display=swap'); @keyframes bounce{0%,80%,100%{transform:scale(0.7);opacity:0.4}40%{transform:scale(1.1);opacity:1}}`}</style>
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.8rem", background:"linear-gradient(135deg,#d4aaff,#ff9ed2)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>StudyFlow</div>
        <div style={{ display:"flex", gap:8 }}>
          {[0,1,2].map(i => <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:["#b47aff","#ff9ed2","#a0c4ff"][i], animation:`bounce 1.2s ${i*0.2}s infinite ease-in-out` }}/>)}
        </div>
        <div style={{ fontSize:"0.82rem", color:"rgba(200,180,255,0.5)" }}>Loading your progress...</div>
      </div>
    );
  }
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif", minHeight:"100vh", background:"#0f0a1e", backgroundImage:"radial-gradient(ellipse at 20% 10%,rgba(180,120,255,0.18) 0%,transparent 50%),radial-gradient(ellipse at 80% 80%,rgba(255,150,200,0.12) 0%,transparent 50%)", color:"#f0eaff", paddingBottom:80 }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(180,120,255,0.5)}50%{box-shadow:0 0 0 8px rgba(180,120,255,0)}}
        @keyframes completePop{0%{transform:scale(1)}50%{transform:scale(1.06)}100%{transform:scale(1)}}
      `}</style>
      ...
      {showCelebration && (
        <div style={{ position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",background:"linear-gradient(135deg,rgba(15,10,30,0.97),rgba(40,20,80,0.97))",border:"1px solid rgba(180,120,255,0.5)",borderRadius:24,padding:"40px 52px",textAlign:"center",zIndex:999,animation:"popIn 0.4s cubic-bezier(0.34,1.56,0.64,1)",boxShadow:"0 30px 80px rgba(180,120,255,0.4)" }}>
          ...
          <div style={{ fontSize:"0.82rem",color:"rgba(200,180,255,0.7)" }}>Progress saved! You're doing amazing 💜</div>
        </div>
      )}
      ...
      <div style={{ background:"rgba(255,255,255,0.04)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(180,120,255,0.2)",padding:"20px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100 }}>
        <div style={{ fontFamily:"'Playfair Display',serif",fontSize:"1.5rem",fontWeight:700,background:"linear-gradient(135deg,#d4aaff,#ff9ed2,#a0c4ff)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent" }}>Study<em>Flow</em></div>
        <div style={{ display:"flex",alignItems:"center",gap:16 }}>
          <div style={{ background:"rgba(255,180,100,0.15)",border:"1px solid rgba(255,180,100,0.35)",borderRadius:20,padding:"5px 14px",fontSize:"0.78rem",fontWeight:600,color:"#ffcc88" }}>🔥 Keep going!</div>
      ...
      {completedCount > 0 && (
        <div style={{ margin:"12px 28px 0",display:"flex",flexWrap:"wrap",gap:8 }}>
          {DAILY_PLAN.flatMap(d => d.topics).filter(t => completedTopics[t.id]).map(t => (
            <div key={t.id} style={{ display:"flex",alignItems:"center",gap:6,background:"rgba(123,232,160,0.08)",border:"1px solid rgba(123,232,160,0.25)",borderRadius:20,padding:"4px 12px",fontSize:"0.72rem",color:"#7be8a0" }}>
              ✓ {t.title.length > 30 ? t.title.slice(0,30)+"…" : t.title}
              <span onClick={() => handleUnmark(t.id)} style={{ cursor:"pointer",opacity:0.5,marginLeft:2,fontSize:"0.65rem" }}>✕</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ padding:"22px 28px 0" }}>
        ...
        <div style={{ fontSize:"0.8rem",color:"rgba(200,180,255,0.5)" }}>{todayPlan?.topics.length} topic{todayPlan?.topics.length > 1?"s":""} today · Tap a card to open notes</div>
      </div>
      ...
      {allDayDone && (
        <div style={{ margin:"14px 28px 0",background:"rgba(123,232,160,0.08)",border:"1px solid rgba(123,232,160,0.28)",borderRadius:12,padding:"12px 20px",textAlign:"center",fontSize:"0.83rem",color:"#7be8a0",fontWeight:600 }}>
          ✅ All topics for today completed! Progress saved! Move to the next day 🌟
        </div>
      )}
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:14,padding:"18px 28px 0" }}>
        {todayPlan?.topics.map(topic => {
          ...
          return (
            <div key={topic.id} style={{ background:active?"rgba(180,120,255,0.12)":done?"rgba(123,232,160,0.05)":"rgba(255,255,255,0.03)",border:`1px solid ${active?"#b47aff":done?"rgba(123,232,160,0.38)":"rgba(180,120,255,0.18)"}`,borderRadius:18,padding:22,cursor:"pointer",transition:"all 0.28s",animation:"fadeUp 0.4s ease",boxShadow:active?"0 0 0 1px rgba(180,120,255,0.3),0 12px 40px rgba(180,120,255,0.22)":"none" }}
              ...
              {done && <div style={{ marginTop:10,fontSize:"0.75rem",color:"#7be8a0",fontWeight:600 }}>✓ Completed & Saved</div>}
              <div style={{ marginTop:14,width:"100%",padding:"9px 0",borderRadius:10,background:active?"linear-gradient(135deg,rgba(180,120,255,0.45),rgba(255,150,200,0.3))":"linear-gradient(135deg,rgba(180,120,255,0.22),rgba(255,150,200,0.14))",border:"1px solid rgba(180,120,255,0.35)",color:"#f0eaff",fontSize:"0.8rem",fontWeight:600,textAlign:"center" }}>
        ...
        <div style={{ margin:"24px 28px 0",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(180,120,255,0.2)",borderRadius:22,overflow:"hidden",animation:"fadeUp 0.35s ease" }}>
          ...
          <div style={{ padding:"18px 24px",background:"rgba(180,120,255,0.08)",borderBottom:"1px solid rgba(180,120,255,0.15)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10 }}>
              ...
              <button onClick={handleCopy} style={{ padding:"8px 18px",borderRadius:10,fontSize:"0.8rem",fontWeight:600,fontFamily:"'DM Sans',sans-serif",cursor:canCopy?"pointer":"not-allowed",border:"none",background:copied?"linear-gradient(135deg,#7be8a0,#4ecdc4)":canCopy?"linear-gradient(135deg,#b47aff,#ff9ed2)":"rgba(255,255,255,0.07)",color:canCopy||copied?"#0f0a1e":"rgba(200,180,255,0.4)",transition:"all 0.2s",animation:canCopy&&!copied?"pulse 2s infinite":"none" }}>
                {copied ? "✓ Copied!" : canCopy ? "📋 Copy" : "⏳ Wait..."}
              </button>
            </div>
          </div>

          {/* Notes Body */}
          <div style={{ padding:"24px 28px",maxHeight:500,overflowY:"auto" }}>
            {renderNotes(notes)}
          </div>

          {/* ── MARK AS COMPLETE BUTTON ── */}
          <div style={{ padding:"20px 28px",borderTop:"1px solid rgba(180,120,255,0.15)",background:"rgba(180,120,255,0.05)" }}>
            {completedTopics[selectedTopic.id] ? (
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12 }}>
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  <div style={{ width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#7be8a0,#4ecdc4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.1rem" }}>✓</div>
                  <div>
                    <div style={{ fontSize:"0.88rem",fontWeight:600,color:"#7be8a0" }}>Topic completed & saved!</div>
                    <div style={{ fontSize:"0.72rem",color:"rgba(200,180,255,0.5)" }}>Your progress is stored in the database 💜</div>
                  </div>
                </div>
                <button onClick={() => handleUnmark(selectedTopic.id)} style={{ padding:"7px 16px",borderRadius:10,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(200,180,255,0.5)",fontSize:"0.76rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif" }}>
                  {savingId === selectedTopic.id ? "..." : "Undo"}
                </button>
              </div>
            ) : (
              <div style={{ textAlign:"center" }}>
                <button
                  ...
                  disabled={savingId === selectedTopic.id}
                  style={{ width:"100%",padding:"14px 0",borderRadius:14,background:"linear-gradient(135deg,#7be8a0,#4ecdc4)",border:"none",color:"#0a1a0f",fontSize:"1rem",fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:savingId===selectedTopic.id?"not-allowed":"pointer",transition:"all 0.25s",display:"flex",alignItems:"center",justifyContent:"center",gap:10,animation:"completePop 0.3s ease",boxShadow:"0 8px 24px rgba(123,232,160,0.3)" }}>
                  {savingId === selectedTopic.id ? (
                    <><div style={{ width:18,height:18,border:"2px solid rgba(0,0,0,0.3)",borderTop:"2px solid #0a1a0f",borderRadius:"50%",animation:"spin 0.8s linear infinite" }}/> Saving...</>
                  ) : (
                    <> ✅ Mark as Complete — Save Progress</>
                  )}
                </button>
                <div style={{ marginTop:8,fontSize:"0.72rem",color:"rgba(200,180,255,0.4)" }}>Clicking this saves your progress permanently to the database</div>
              </div>
            )}
          </div>
        </div>
